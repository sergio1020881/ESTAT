...
com skew = -0,1051 e kurt = -0,4016 \\
Na Região \textbf{B} a Média < Moda = Mediana \\
com skew = 0,1119 e kurt = -0,1835 \\
...
$\therefore$\\
$IC_{A_{0.95}}=\left[ \quad 18.8877 \: , \: 21.1956 \quad \right]$ \hspace{1cm} and \hspace{1cm} $IC_{B_{0.95}}=\left[ \quad 20.4519 \: , \: 22.6314 \quad \right]$\\
\\
\noindent
...
\hspace{3cm} $\Longrightarrow$ \hspace{3cm}
\begin{minipage}[l]{0pt}
\[\bar{Y}_{A,B} \sim N \big(\mu_{A,B};\delta^2_{A,B}\big)\]
\end{minipage}\\
\\
\\
$P(X_A < 6)=P(X_A \leqslant 5) \cong 0,2429 $  \quad e \quad $P(X_B < 6)=P(X_B \leqslant 5) \cong 0,1589$ \\
\\
...
\\
$\hat{P_A}-\hat{P_B} \sim N \left( 0,0839 \; ; \; 0,003528\right)$ \hspace{1cm}
$z_{(1-\frac{\alpha}{2})}=\phi^{-1}(0,985)=2,1701$ \\
\\
Recorrendo a calculadaora casio $fx-9860GII$ : \\
\\
$\triangle= InvNorm(0.985)\sqrt{\frac{0.2429(1-0.2429)}{90}+\frac{0.1589(1-0.1589)}{90}}\: \cong \:0.1289$
\\
$\therefore$
\\
$IC_{97\%}(\hat{P_A}-\hat{P_B})=\left[ \quad 0,0839 \:-\: 0,1289 \: ; \: 0,0839 \:+\: 0,1289 \quad \right] \: = \: \left[ \quad -0,045 \:;\: 0,2128 \quad \right] $ \\
\\
A Diferença de proporção é de 0,0839 com uma margen de erro de 0,1289, que é bastante significativo pois pode atingir 21,28\% no limit superior. \\
\subsection{$TESTE \quad REG$}
\noindent
Testar se a região (REG) tem umefeito estatisticamente significativo sobre TEE e NEE ao nível de diferença de médias. Considrere uma significância à sua escolha inferior ou igual a 5\%. Use o critério do valor de prova para fundamentar a decisão.
