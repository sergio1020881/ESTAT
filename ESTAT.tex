...
\noindent
Na tabela acima a mediana e a moda são estimativas aproximadas, apenas nos da uma dica na qual esta na classe representativa. \\
A mediana pode ser obtida pela frequencia acumulativa quando esta é igual a 50\%, ou seja, $F_i(Mediana)=0,5$ \\
A Moda mais facilmente identificada pelo seu pico maximo na lista de amostras.
...
\\
A Diferença de proporção tem um erro de 36,24\%. \\
...
\end{array}\right.$$
\end{minipage}
\hspace{2cm} $\Longrightarrow$ \hspace{1cm}
\begin{minipage}[l]{0pt}
\[\bar{X}=\frac{\sum_{i=1}^nX_i}{n}\sim N \big(\mu;\frac{\delta^2}{n}\big)\]
\end{minipage}\\
\\
...
$  z_0\:=\: \frac{\bar{x}_A-\bar{x_B}}{\sqrt{\frac{\delta_A^2}{n_A}+\frac{\delta_B^2}{n_B}}} $\\
$RC_z=]-\infty,-z_{1-\alpha}]$\\
$ pvalue=P(Z<z_0) $\\
\\
Condição REE:\\
$ \bar{y_A}-\bar{y_B} $\\
$ \frac{\delta_A^2}{n_A}+\frac{\delta_B^2}{n_B} $\\
$  z_0\:=\: \frac{\bar{y}_A-\bar{y_B}}{\sqrt{\frac{\delta_A^2}{n_A}+\frac{\delta_B^2}{n_B}}} $\\
$ RC_z=]-\infty,-z_{1-\alpha}] $\\
$ pvalue=P(Z<z_0) $\\
\\
